...
Vector4 ForwardKinematic::computeLocalQuaternion(Joint* joint)
{
	...
	*/
	...
	sz = sin(z / 2);
	*/
	qx = buildQuaternionRotation(x,1,0,0);
	qy = buildQuaternionRotation(y, 0, 1, 0);
	qz = buildQuaternionRotation(z, 0, 0, 1);
	switch (joint->rotationOrder)
	{
		//enum { NONE = 0, ZYX = 1, YZX = 2, ZXY = 3, XZY = 5, YXZ = 6, XYZ = 7 };
	case 1:
...
Vector4 ForwardKinematic::computeGlobalPosition(Joint* joint)
{
	...
	Vector4 q_inverse = Vector4(-joint->parent->Globalquat.x, -joint->parent->Globalquat.y, -joint->parent->Globalquat.z, joint->parent->Globalquat.w);
	Vector4 rotated = quaternionMultiplication(quaternionMultiplication(joint->Globalquat, joint->LocalPos), q_inverse);
	pos = joint->parent->GlobalPos + rotated;
	joint->GlobalPos = pos;
	return joint->GlobalPos;
}
